...
module "gke" {
  ...
  maintenance_start_time          = var.maintetenance_start_time
  maintenance_end_time            = var.maintetenance_end_time
  maintenance_recurrence          = var.maintetenance_recurrence
...
module "k8s-proxy" {
  source                       = ".//modules/k8s-proxy"
  gke_region                   = var.region
  gke_project_id               = var.project_id
  cluster_name                 = local.cluster_name
  vpc_project_id               = module.vpc.network_name
#  project_sa_credentials       = local.project_sa_credentials
  helm_release_name            = var.k8_proxy_helm_release_name
